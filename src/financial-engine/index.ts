...
export function impulseImpact(amount:number,debts:Debt[],extra:number,strategy:StrategyKind){const base=projectDebts(debts,extra,strategy),boosted=projectDebts(debts,extra+Math.max(0,amount),strategy),remaining=debts.filter(d=>d.status==='active'&&d.currency==='ARS').reduce((sum,d)=>sum+d.balance,0);return{debtPercent:remaining>0?amount/remaining*100:0,daysAdvanced:base.months!==null&&boosted.months!==null?Math.max(0,(base.months-boosted.months)*30):null}}
