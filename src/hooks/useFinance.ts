...
export function useFinance() {
  ...
  const settings=settingsRow??defaultSettings, budget=calculateBudget(incomes,expenses,debts), progress=calculatePortfolioProgress(debts,payments), projection=projectDebts(debts,budget.availableExtra,settings.strategy,new Date(),1200,{mandatoryAffordable:budget.deficit===0})
