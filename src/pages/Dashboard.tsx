...
export function Dashboard(){
  ...
  const active=useMemo(()=>finance.debts.filter(debt=>debt.status==='active'&&debt.balance>0),[finance.debts]),next=useMemo(()=>[...active].sort((a,b)=>dueRank(a)-dueRank(b)),[active]),selected=finance.debts.find(debt=>debt.id===paymentDebt),ars=total(finance.debts,'ARS'),usd=total(finance.debts,'USD'),sameCurrency=new Set(finance.debts.map(debt=>debt.currency)).size===1?finance.debts[0]?.currency:null
  ...
  const progressFormat=finance.progress.method==='equal_debt_average'?undefined:(value:number)=>formatMoney(value,finance.progress.currency??sameCurrency??'ARS')
  return <Page title="Tu plan" eyebrow="Hoy" action={<button className="button primary dashboard-payment" disabled={!active.length} onClick={openPayment}>+ Registrar pago</button>}>
    <section aria-label="Progreso total" className="dashboard-progress"><ProgressTrack percent={finance.progress.progressPercent} character={finance.settings.progressCharacter} paid={finance.progress.totalPaidPrincipal} remaining={finance.progress.currentOutstanding} formatAmount={progressFormat} title="Progreso total" approximateAmounts={finance.progress.method==='equal_debt_average'}/><button className="button primary large dashboard-payment mobile-payment" disabled={!active.length} onClick={openPayment}>+ Registrar pago</button></section>
    ...
    {modal==='complete'&&<Modal title="Deuda terminada" onClose={()=>setModal(null)}><div className="completion"><span>✓</span><h2>Ese saldo llegó a cero.</h2><p>La deuda quedó pagada con la fecha de hoy. El progreso total ya incorpora este cierre.</p><button className="button primary" onClick={()=>setModal(null)}>Seguir</button></div></Modal>}
  </Page>
}
