...
export async function ensureDB() { const now = new Date().toISOString(); await db.transaction('rw', db.meta, db.settings, async () => { if (!await db.meta.get('main')) await db.meta.put({ id:'main', schemaVersion:2, createdAt:now, updatedAt:now }); if (!await db.settings.get('main')) await db.settings.put({ id:'main', ...defaultSettings }) }) }
